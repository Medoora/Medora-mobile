...
export class StorageService {
  ...
  static async hasEnoughSpace(userId: string, fileBytes: number): Promise<boolean> {
    const storage = await this.getUserStorage(userId);
    if (!storage) return true; // Assume yes if no storage record
    
    const currentTotal = storage.totalBytes;
    const quotaBytes = storage.quotaBytes;
    
    return (currentTotal + fileBytes) <= quotaBytes;
  }

  // Get remaining storage space
  static async getRemainingSpace(userId: string): Promise<number> {
    const storage = await this.getUserStorage(userId);
    if (!storage) return DEFAULT_QUOTA_BYTES;
    
    return Math.max(0, storage.quotaBytes - storage.totalBytes);
  }

  // Get storage usage percentage
  static async getUsagePercentage(userId: string): Promise<number> {
    const storage = await this.getUserStorage(userId);
    if (!storage) return 0;
    
    return (storage.totalBytes / storage.quotaBytes) * 100;
  }
}